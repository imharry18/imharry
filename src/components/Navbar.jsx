...
const Navbar = () => {
  ...
  return (
    <>
      {/* --- Navigation Bar --- */}
      <header 
        className="fixed top-0 left-0 w-full z-50 px-4 sm:px-6 py-3 shadow-lg backdrop-blur-md
      ...
      >
        <nav className="container mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center space-x-2">
            ...
            />
            <a className='pl-2 sm:pl-3' href="#home" onClick={() => setActiveLink('Home')}>
              <SignatureLogo />
            </a>
          </div>

          {/* Center: Desktop Menu */}
          <div className="hidden md:flex items-center bg-black/30 backdrop-blur-sm px-3 py-2 rounded-full space-x-1 lg:space-x-2">
            {navLinks.map((item) => (
              <a 
                key={item.name}
                href={`#${item.id}`}
